...
char *stringize_arg(va_list list, specifier spec, unsigned int *freeflag)
{
	...
	case 'i':
		*freeflag = 1;
		...
		if (spec.length == -1)
			return (prep_numeric(sitos(list), spec));
		if (spec.length < -1)
			return (prep_numeric(ssitos(list), spec));
		return (prep_numeric(itos(list), spec));
	case 'b':
		*freeflag = 1;
		if (spec.length == 1)
			return (prep_numeric(litobin(list), spec));
		if (spec.length == -1)
			return (prep_numeric(sitobin(list), spec));
		if (spec.length < -1)
			return (prep_numeric(ssitobin(list), spec));
		return (prep_numeric(itobin(list), spec));
	case 'u':
		*freeflag = 1;
		if (spec.length == 1)
			return (prep_numeric(lutos(list), spec));
		if (spec.length == -1)
			return (prep_numeric(sutos(list), spec));
		if (spec.length < -1)
			return (prep_numeric(ssutos(list), spec));
	...
	case 'o':
		*freeflag = 1;
		...
		if (spec.length == -1)
			return (prep_numeric(sitoo(list), spec));
		if (spec.length < -1)
			return (prep_numeric(ssitoo(list), spec));
		return (prep_numeric(itoo(list), spec));
	case 'x':
		*freeflag = 1;
		if (spec.length == 1)
			return (prep_numeric(litohex(list), spec));
		if (spec.length == -1)
			return (prep_numeric(sitohex(list), spec));
		if (spec.length < -1)
			return (prep_numeric(ssitohex(list), spec));
		return (prep_numeric(itohex(list), spec));
	case 'X':
		*freeflag = 1;
		if (spec.length == 1)
			return (prep_numeric(lhtoH(list), spec));
		if (spec.length == -1)
			return (prep_numeric(shtoH(list), spec));
		if (spec.length < -1)
			return (prep_numeric(sshtoH(list), spec));
		return (prep_numeric(htoH(list), spec));
	case 'r':
		*freeflag = 1;
